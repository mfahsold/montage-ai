...
*   **No hardcoded values**: Avoid hardcoding registry URLs, IPs, file paths, resource limits, or credentials. Add new deployment settings to `deploy/config.env` or `deploy/k3s/config-global.yaml.example` (users copy to `config-global.yaml`), and runtime defaults to `config.Settings`. Run `./scripts/check-hardcoded-registries.sh` and pre-push checks before committing.
