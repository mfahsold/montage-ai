...
def render_shard(
    ...
    def _write_shard_report(
        clip_count: int, segments: Optional[List[str]] = None
    ) -> None:
    ...
    logger.info(
        f"   🎬 Shard {shard_index}/{shard_count} rendering {len(my_clips)} clips (indices {start_idx}-{end_idx - 1})"
    )
    ...
    for i, clip_dict in enumerate(my_clips):
        ...
        logger.info(
            f"   ✂️ Shard {shard_index}: Rendering clip {i + 1}/{len(my_clips)}: {os.path.basename(meta.source_path)}"
        )
        temp_path = os.path.join(shard_output_dir, f"clip_{shard_index}_{i}.mp4")

        try:
            from ..ffmpeg_config import get_config, STANDARD_PRESET

            cfg = get_config()

            hwaccel = []
            if (
                os.environ.get("FFMPEG_HWACCEL")
                and os.environ.get("FFMPEG_HWACCEL") != "none"
            ):
            ...
            width = int(
                os.environ.get(
                    "EXPORT_WIDTH", getattr(export_cfg, "resolution_width", 1920)
                )
            )
            height = int(
                os.environ.get(
                    "EXPORT_HEIGHT", getattr(export_cfg, "resolution_height", 1080)
                )
            )
            ...
            ffmpeg_cmd = (
                ...
                + [
                    ...
                    "-c:v",
                    os.environ.get("FFMPEG_ENCODER", cfg.codec),
                    "-crf",
                    str(settings.encoding.crf),
                    "-preset",
                    STANDARD_PRESET,
                    "-c:a",
                    "aac",
    ...
    # Save shard report
    segments = (
        [s.path for s in renderer.segment_writer.segments]
        if hasattr(renderer.segment_writer, "segments")
        else []
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Montage AI Distributed Renderer Worker"
    )
    parser.add_argument(
        "--clips-json",
        required=True,
        help="Path to JSON file containing ClipMetadata list",
    )
    parser.add_argument("--shard-index", type=int, default=None)
    parser.add_argument("--shard-count", type=int, default=None)
    parser.add_argument(
        "--output-dir", required=True, help="Shared directory for segments"
    )
    ...
    parser.add_argument(
        "--quality", default="standard", choices=["preview", "standard", "high"]
    )
