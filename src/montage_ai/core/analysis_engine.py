...
def _log_resource_usage(phase: str, progress_pct: int = 0) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"   📊 {phase}{progress_str}: CPU process={cpu_pct:.1f}% sys={sys_cpu:.1f}% | Memory process={mem_mb:.0f}MB sys={sys_mem_pct:.1f}%{gpu_str}"
        )
    except Exception as e:
...
class AssetAnalyzer:
    ...
    def determine_output_profile(self) -> None:
        ...
        from ..video_metadata import (
            determine_output_profile as _determine_profile,
            OutputProfile,
        )
        ...
        if has_explicit_width or has_explicit_height:
            try:
                ...
                w = int(
                    os.environ.get(
                        "EXPORT_WIDTH", getattr(export_cfg, "resolution_width", 0) or 0
                    )
                )
                h = int(
                    os.environ.get(
                        "EXPORT_HEIGHT",
                        getattr(export_cfg, "resolution_height", 0) or 0,
                    )
                )
                if w > 0 and h > 0:
    ...
    def analyze_music(self, isolated_audio_path: Optional[str] = None) -> None:
        ...
        if (
            ...
            if match:
                ...
                logger.warning(
                    f"   ⚠️ Requested track '{requested_track}' not found, falling back to variant default"
                )
        ...
        try:
            ...
            if not is_cgpu_available():
                if self.settings.features.strict_cloud_compute:
                    raise RuntimeError(
                        "Strict cloud compute enabled: cgpu voice isolation not available."
                    )
                logger.warning(
                    "   Voice isolation requires cgpu - using original audio"
                )
                return audio_path
            ...
            if result.success:
                ...
                if stems.get("no_vocals"):
                    logger.info(
                        f"   ✅ Voice isolated: Using accompaniment for analysis: {stems['no_vocals']}"
                    )
    ...
    def detect_scenes(self, progress_callback: Optional[Any] = None) -> None:
        ...
        if len(video_files) > 100:
            logger.info(
                f"   ⚠️ MASSIVE footage detected ({len(video_files)} clips). Switching to Intelligent Sampling..."
            )
            ...
            if len(video_files) > 75:
                sampled = (
                    video_files[:25]
                    + video_files[-25:]
                    + rng.sample(video_files[25:-25], 25)
                )
                ...
                logger.info(
                    f"   🎯 Sampled down to {len(video_files)} high-priority clips for analysis."
                )
        ...
        if uncached_videos and self.settings.stabilization.cluster_mode:
            try:
                detected_scenes = self._detect_scenes_distributed(
                    uncached_videos, threshold, progress_callback
                )
            except (RuntimeError, ConnectionError, TimeoutError) as e:
                logger.warning(
                    f"Distributed scene detection failed ({e}), falling back to local processing"
                )
        ...
        if uncached_videos and not detected_scenes:
            ...
            if self.settings.stabilization.low_memory_mode:
                ...
                max_workers = 1
                logger.info(
                    "   ⚠️ LOW_MEMORY_MODE: Sequential scene detection (1 worker)"
                )
            else:
            ...
            logger.info(
                f"   🚀 ProcessPool scene detection ({len(uncached_videos)} videos, {max_workers}/{cpu_count} CPU workers)"
            )
            ...
            try:
                ...
                per_file_timeout = int(
                    getattr(
                        ...
                        "scene_detection_per_file_timeout_seconds",
                        120,
                    )
                )
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    futures = {
                        executor.submit(_detect_video_scenes_worker, v, threshold): v
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            try:
                                ...
                                pass
                            logger.warning(
                                "Scene detection timed out for %s (>%ss); skipping",
                                os.path.basename(v),
                                per_file_timeout,
                            )
                            try:
                            ...
                            logger.warning(
                                "   ⚠️ Scene detection failed for %s: %s",
                                os.path.basename(v),
                                exc,
                            )
                            try:
                        ...
                        try:
                            ...
                            logger.debug(
                                "Failed to save scenes to cache for %s", v_path
                            )
                        ...
                        if now - last_log_time >= 5.0:
                            snapshot = _log_resource_usage(
                                "Scene Detection", progress_pct
                            )
                            last_log_time = now
                ...
                logger.warning(
                    f"   ⚠️ ProcessPool failed ({e}), falling back to ThreadPool"
                )
                ...
                with ThreadPoolExecutor(max_workers=thread_workers) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        if now - last_log_time >= 5.0:
                            snapshot = _log_resource_usage(
                                "Scene Detection (ThreadPool)", progress_pct
                            )
                            last_log_time = now
        ...
        if cache_hits > 0:
            logger.info(
                f"   ⚡ Used cached scene detection for {cache_hits}/{len(video_files)} videos"
            )
        ...
        if max_analyze > 0 and len(self.ctx.media.all_scenes) > max_analyze:
            logger.info(
                f"   🤖 AI Director is watching footage... (analyzing {max_analyze}/{len(self.ctx.media.all_scenes)} scenes)"
            )
            scenes_to_analyze = self.ctx.media.all_scenes[:max_analyze]
        else:
            logger.info(
                f"   🤖 AI Director is watching all {len(self.ctx.media.all_scenes)} scenes..."
            )
                ...
                for future in as_completed(futures):
                    ...
                    if self.ctx.job_id:  # Only if we have a job context
                        ...
                        if completed_ai % max(1, total_ai // 10) == 0:
                            logger.info(
                                f"   🤖 AI Analysis: {completed_ai}/{total_ai} ({int(completed_ai / total_ai * 100)}%)"
                            )
        ...
        self.ctx.media.all_scenes_dicts = [
            s.to_dict() for s in self.ctx.media.all_scenes
        ]

        logger.info(
            f"   📹 Found {len(self.ctx.media.all_scenes)} scenes in {len(video_files)} videos"
        )
    ...
    def _detect_scenes_distributed(
        self,
        ...
        logger.info(
            f"   🌐 Cluster Mode: Submitting distributed scene detection job for {len(video_paths)} videos..."
        )
        ...
        parallelism = min(
            len(video_paths), self.settings.stabilization.cluster_parallelism
        )

        try:
            job = submitter.submit_scene_detection(
                video_paths=video_paths, parallelism=parallelism, threshold=threshold
            )
            ...
            poll_interval = max(
                1, int(self.settings.cluster.status_poll_interval_seconds)
            )
            request_timeout = max(
                1, int(self.settings.cluster.status_request_timeout_seconds)
            )

            final_status = None
            while True:
                try:
                    status = submitter.get_job_status(
                        job.name, request_timeout=request_timeout
                    )
                except Exception as e:
                    status_errors += 1
                    logger.warning(
                ...
                if status.succeeded > last_succeeded:
                    logger.info(
                        f"   ⏳ Progress: {status.succeeded}/{parallelism} workers completed"
                    )
            ...
            if not final_status or not final_status.is_successful:
                ...
                raise RuntimeError(
                    f"Distributed job {job.name} failed (failed={total_failed})"
                )
    ...
    def _get_files(self, directory: str, extensions: Iterable[str]) -> List[str]:
        ...
        if not root.exists():
            return []
        return [
            str(path) for path in list_media_files(root, extensions, recursive=True)
        ]
