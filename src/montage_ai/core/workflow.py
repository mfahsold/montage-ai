...
class VideoWorkflow(ABC):
    ...
    def execute(self) -> WorkflowResult:
        ...
        try:
            ...
            logger.info(
                f"[{self.workflow_name}] Initializing job {self.options.job_id}"
            )
            self.initialize()
            ...
            try:
                ...
                logger.exception(
                    "Failed to persist workflow result for %s", self.options.job_id
                )

            logger.info(
                f"[{self.workflow_name}] Completed in {duration:.1f}s: {output_path}"
            )
            return self._result
            ...
            self._result = WorkflowResult(success=False, error=error_msg)
    ...
    def _update_phase(
        ...
        try:
            self.job_store.update_job_with_retry(
                self.options.job_id, updates, retries=3
            )
        except (ConnectionError, TimeoutError, RuntimeError):
            logger.exception(
                "Failed to persist workflow phase update for %s", self.options.job_id
            )
    ...
    def _update_progress(
        ...
        try:
            if percent >= 90:
                ...
                self.job_store.update_job_with_retry(
                    self.options.job_id, updates, retries=2
                )
            else:
            ...
            logger.exception(
                "Failed to persist progress update for %s", self.options.job_id
            )
