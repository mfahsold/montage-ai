...
DEPRECATED: This module was created for a refactoring initiative that was never
completed. The functions here are not used in the current codebase.

Original intent was to consolidate common scene analysis functions from:
  - scene_analysis.py
  - core/analysis_engine.py
  - core/video_analysis_engine.py

Status: UNFINISHED REFACTORING - Keep for reference only
Future: Consider integrating these utilities into scene_analysis.py
...
warnings.warn(
    "scene_helpers.py is deprecated. This was an unfinished refactoring initiative. "
    "Functions are not used in current codebase. Consider using scene_analysis.py directly.",
    DeprecationWarning,
    stacklevel=2,
)
...
class SceneProcessor:
    ...
    def normalize_scene(
        ...
        scene_id = (
            scene.get("id") or scene.get("scene_id") or f"scene_{int(start * 1000)}"
        )
